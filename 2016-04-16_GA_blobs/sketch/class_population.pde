public class Population{
	...
	public Population(int num, float mutation_rate){
		...
		int record = 0;
		for(Organism o : this.ORGANISMS) if(o.gene_iterator > record) record = o.gene_iterator;
		println("Max Genes needed : " + record);
	}
	...
	public void display(){
		int index = 0;
		float size = sqrt(this.ORGANISMS.length);
		for(int cols=0; cols<size; cols++){
			for(int rows=0; rows<size; rows++){
				Organism o = this.ORGANISMS[index++];
				int
					x = int(map(cols, 0, size, 50, width + 50)),
					y = int(map(rows, 0, size, 50, height + 50));
